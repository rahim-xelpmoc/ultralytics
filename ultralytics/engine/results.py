...
class ResultsWithEmbeddings(SimpleClass):
    """
    A class for storing and manipulating inference results.
    ...
    def __init__(
        ...
        self.orig_img = orig_img
        self.orig_shape = orig_img.shape[:2]
        self.boxes = Boxes(boxes[:,:6], self.orig_shape) if boxes is not None else None  # native size boxes
        self.masks = Masks(masks, self.orig_shape) if masks is not None else None  # native size or imgsz masks
        self.probs = Probs(probs) if probs is not None else None
        self.keypoints = Keypoints(keypoints, self.orig_shape) if keypoints is not None else None
        self.obb = OBB(obb, self.orig_shape) if obb is not None else None
    ...
    def update(self, boxes=None, masks=None, probs=None, obb=None, keypoints=None):
        ...
        """
        if boxes is not None:
            self.boxes = Boxes(ops.clip_boxes(boxes[:,:6], self.orig_shape), self.orig_shape)
        if masks is not None:
            self.masks = Masks(masks, self.orig_shape)
        if probs is not None:
            self.probs = probs
